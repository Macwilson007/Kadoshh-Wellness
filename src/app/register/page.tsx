  ...
  return (
    <div className="min-h-screen bg-stone-50/50 flex items-center justify-center pt-32 pb-20 px-4">
      <div className="bg-white p-8 rounded-sm shadow-sm border border-stone-200 max-w-md w-full">
        <h1 className="text-3xl font-light mb-6 text-stone-800 text-center uppercase tracking-widest">Join Us</h1>
        {error && <p className="text-red-500 mb-4 text-sm text-center">{error}</p>}
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-xs uppercase tracking-tighter text-stone-500 mb-1">Full Name</label>
            <input
              type="text"
              value={name}
        ...
        <p className="mt-6 text-center text-sm text-stone-500">
          ...
          <Link href="/login" className="text-stone-800 underline underline-offset-4 decoration-stone-200 hover:decoration-stone-800 transition-colors">
            Sign In
          </Link>
        </p>
      </div>
